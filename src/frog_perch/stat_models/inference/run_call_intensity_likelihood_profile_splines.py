...
(Likelihood Profile + P-Spline Version).
"""
...
from frog_perch.stat_models.inference.fit_call_intensity_likelihood_profile_splines import (
    run_call_intensity_pipeline,
    print_diagnostics,
)

# You can update this default path if you have a new config file location
CONFIG_PATH = "/home/breallis/dev/frog_perch/src/frog_perch/stat_models/inference/call_intensity_likelihood_profile_splines.yaml"
...
def main():
    parser = argparse.ArgumentParser(description="Run Stan call-intensity model (Likelihood Profile + Splines).")
    ...
    parser.add_argument("--knot-spacing-season", type=float, dest="knot_spacing_season_days", 
                        help="Days between seasonal knots (e.g. 3.0).")
    parser.add_argument("--knot-spacing-diel", type=float, dest="knot_spacing_diel_min", 
                        help="Minutes between diel knots (e.g. 10.0).")
    ...
    defaults = {
        ...
        # Spline Defaults (Updated from HSGP M=10)
        "knot_spacing_season_days": 3.0,  # Every 3 days
        "knot_spacing_diel_min": 10.0,    # Every 10 minutes
    ...
    print("Running Call-Intensity Likelihood Profile Pipeline (Splines) with configuration:")
    for k, v in cfg.items():
        print(f"  {k}: {v}")

    # --- Run Pipeline ---
    df, window_metadata, spline_params, fit = run_call_intensity_pipeline(
        csv_dir=cfg["csv_dir"],
    ...
    # 2. Save Spline Parameters (for reconstruction)
    np.savez(
        cfg["output_dir"] / "spline_params.npz",
